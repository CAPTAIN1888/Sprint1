...
# Model to store shipping address information linked to a user
...
# Signal handler to automatically create a ShippingAddress instance when a User is created
    
def create_shipping(sender, instance, created, **kwargs):
    if created:
        user_shipping = ShippingAddress(user=instance)
        user_shipping.save()
# Connect the post_save signal of User model to the create_shipping function
...
class Order(models.Model):
    ...
    # Link order to user, can be null for guest checkouts
    user = models.ForeignKey(User,on_delete=models.CASCADE,null=True, blank=True)
